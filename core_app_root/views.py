...
from elibraryplatform.forms import UserSelection
...
def captureindex(request):
    context = {'forms': UserSelection }
    return render(request, 'captureindex.html', context)
...
def create_dataset(request):
    if request.method == "POST":
        ...
        face_detector = cv2.CascadeClassifier('/ml/haarcascade_frontalface_default.xml')  # For each person, enter one numeric face id
...
#         return redirect('capture_face', user_id=request.user.id)
#     else:
...
def capture_face(request):
    ...
    if request.method == 'POST':
        ...
        return redirect('core_app_root:login_with_face')
...
def login_with_face(request):
    ...
    return render(request, 'login_with_face.html')